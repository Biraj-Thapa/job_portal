...
const JobCard = ({ job, onApply, onClickCard, forEmployer }) => {
  return (
    <div
      onClick={() => onClickCard && onClickCard(job.id)}
      className="relative bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 p-6"
    >
        ...
        {forEmployer ? (
          <button className="px-4 py-2 text-sm rounded-lg font-medium border border-gray-300 hover:bg-gray-100 text-gray-800 transition">
           ...
           <button
            onClick={(e) => {
              e.stopPropagation();
              !job.isClosed && onApply && onApply(job.id);
            }}
            disabled={job.isClosed}
            className={`px-5 py-2 text-sm rounded-lg font-medium text-white shadow-sm transition ${
              job.isClosed
                ? "bg-gray-400 cursor-not-allowed"
                : "bg-blue-600 hover:bg-blue-700 hover:shadow-md"
            }`}
          >
            {job.isClosed ? "Closed" : "Apply Now"}
          </button>
        )}
      </div>
    </div>
